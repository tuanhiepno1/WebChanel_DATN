...
import { Modal, Typography, Space, Statistic, message, Button, Divider } from "antd";
...
const unifyVerify = (raw, needAmount, tol) => {
  if (!raw) return null;
  if (raw.ok === false) return { matched: false, status: "ERROR", message: raw.message, transactions: [], totalMatched: 0 };

  // schema mới
  if (Array.isArray(raw?.transactions)) {
    const paid = Number(raw.totalMatched ?? raw.transactions?.[0]?.amount ?? 0);
    return { matched: !!raw.matched, status: raw.status, transactions: raw.transactions, totalMatched: paid };
  }
  ...
  if ("tx" in (raw || {}) || "row" in (raw || {})) {
    const t = raw.tx || {};
    const txs = raw.row
      ? [{ row: raw.row, amount: Number(t.amount || 0), at: t.at, rawCode: t.code, content: t.content }]
      : [];
    const paid = txs[0]?.amount || 0;
    // status tạm, FE sẽ tự phân loại lại
    return { matched: !!raw.matched, status: raw.matched ? "UNKNOWN" : "WRONG_CODE", transactions: txs, totalMatched: paid };
  }

  return { matched: false, status: "WRONG_CODE", transactions: [], totalMatched: 0 };
};
...
const classify = (txs, needAmount, tol) => {
  ...
  if (Math.abs(delta) <= (tol ?? 0)) return { kind: "ok", paid, row: txs[0].row };
  if (delta < 0) return { kind: "underpay", missing: -delta, paid, row: txs[0].row };
...
const VietQRPayModal = ({
  ...
  const claimRow = async (row, orderId) => {
    if (!row) return;
    try {
      await fetch(`${GAS_BASE}/claim?row=${encodeURIComponent(row)}&orderId=${encodeURIComponent(orderId)}`);
    } catch {}
  };
  ...
  const handleVerifyResult = async (resp, needAmount, codeForThis, context) => {
    ...
    const goodTxs = (resp.transactions || []).filter((t) => isExactCodeInTx(t, codeForThis));

    const r = classify(goodTxs, needAmount, context === "topup" ? GAS_TOLERANCE : 0);
    if (r.kind === "ok") {
      if (!firedRef.current && r.row) {
        ...
        await onConfirmTransferred({ paymentCode: codeForThis, amount: needAmount });
        setPhase("done");
        onClose?.();
      }
      return true;
    ...
    if (r.kind === "underpay") {
      if (context === "main") {
        ...
        setPhase("topup");
        message.warning(`Bạn đã chuyển thiếu ${r.missing.toLocaleString()} ₫. Vui lòng chuyển bù.`);
        return true;
      }
      return false;
    }
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (await handleVerifyResult(strict, totalAmount, mainCode, "main")) return;
        ...
        message.error("Không kiểm tra được giao dịch (verify). Kiểm tra cấu hình bridge (GAS_BASE)!");
      }
  ...
  const handleManualConfirm = async () => {
    ...
    try {
      ...
      setPhase("wrongcode");
      message.warning("Không tìm thấy giao dịch phù hợp với mã CK. Vui lòng liên hệ CSKH để rà soát.");
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        const strict = await verifyCodeAmount(topUpCode, topUpAmount, GAS_TOLERANCE);
        // Chỉ accept nếu giao dịch đúng token == topUpCode
        const ok = await handleVerifyResult(strict, topUpAmount, topUpCode, "topup");
        if (ok) {
          ...
          // (do context=topup)
          await onConfirmTransferred({ paymentCode: topUpCode, amount: totalAmount });
          return;
        }
      } catch (e) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, phase, topUpCode, topUpAmount, topUpExpire, totalAmount, onConfirmTransferred, onClose, mainCode]);
  ...
  const renderMainQR = () => (
    <>
      ...
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text>Mã nội dung CK:</Text><Text strong>{mainCode}</Text>
      </Space>

      <div style={{ textAlign: "center", margin: "12px 0" }}>
        <img src={mainQrUrl} alt="VietQR" style={{ width: 500, height: 500, objectFit: "contain", borderRadius: 8 }} />
      </div>

      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text>Số tiền:</Text><Text strong>{Number(totalAmount).toLocaleString()} ₫</Text>
      </Space>

      <div style={{ marginTop: 12 }}>
        <Statistic.Countdown title="Mã QR hết hạn sau" value={mainExpire} onFinish={handleMainExpired} />
      </div>

      <div style={{ marginTop: 12, display: "flex", gap: 8, justifyContent: "flex-end" }}>
        <Button onClick={handleManualConfirm} loading={manualBusy}>
          Tôi đã chuyển khoản
        </Button>
      </div>
    </>
  ...
  const renderTopup = () => (
    <>
      <Title level={5} style={{ marginBottom: 8 }}>Vui lòng chuyển bù</Title>
      <Space style={{ width: "100%", justifyContent: "space-between" }}>
        <Text>Thiếu cần bù:</Text><Text strong>{topUpAmount.toLocaleString()} ₫</Text>
      </Space>
      <Divider />
      {!topUpCode ? (
        <Button
        ...
        <>
          <Space style={{ width: "100%", justifyContent: "space-between" }}>
            <Text>Mã CK bù:</Text><Text strong>{topUpCode}</Text>
          </Space>
          <div style={{ textAlign: "center", margin: "12px 0" }}>
            {topUpQrUrl && <img src={topUpQrUrl} alt="VietQR Topup" style={{ width: 500, height: 500, objectFit: "contain", borderRadius: 8 }} />}
          </div>
          <Space style={{ width: "100%", justifyContent: "space-between" }}>
            <Text>Số tiền bù:</Text><Text strong>{topUpAmount.toLocaleString()} ₫</Text>
          </Space>
          <div style={{ marginTop: 12 }}>
            <Statistic.Countdown title="Mã bù hết hạn sau" value={topUpExpire} onFinish={() => message.warning("Mã bù đã hết hạn, vui lòng tạo lại.")} />
          </div>
        </>
      )}
    </>
  ...
  const renderOverpay = () => {
    ...
    return (
      <>
        <Title level={5} style={{ marginBottom: 8 }}>Bạn đã chuyển <b>thừa</b></Title>
        <Text>Để tránh lệch sổ, hệ thống không tự tạo đơn khi số tiền lớn hơn yêu cầu. Vui lòng liên hệ <b>Zalo CSKH: 0876111815</b> để hoàn/điều chỉnh.</Text>
        <Divider />
        <Space direction="vertical" style={{ width: "100%" }}>
          <Text type="secondary">Thông tin cần cung cấp cho CSKH:</Text>
          <pre style={{ background: "#f6f6f6", padding: 12, borderRadius: 8, whiteSpace: "pre-wrap" }}>{info}</pre>
          <Space>
  ...
  const renderWrongCode = () => {
    ...
    return (
      <>
        <Title level={5} style={{ marginBottom: 8 }}>Không tìm thấy giao dịch phù hợp</Title>
        <Text>Có thể bạn đã nhập <b>sai mã CK</b>, dùng lại mã cũ đã được <b>CSKH xử lý/đã claim</b>, hoặc chưa chuyển khoản. Vui lòng liên hệ <b>Zalo CSKH: 0876111815</b> để được rà soát.</Text>
        <Divider />
        <Space direction="vertical" style={{ width: "100%" }}>
          <Text type="secondary">Thông tin cần cung cấp cho CSKH:</Text>
          <pre style={{ background: "#f6f6f6", padding: 12, borderRadius: 8, whiteSpace: "pre-wrap" }}>{info}</pre>
